...
const locatorMap = new Map<string, Element>();
let _hlEl: Element | null = null;

function injectHlStyle(): void {
  if (document.getElementById('__amhl_style__')) return;
...
function hlElement(uid: string): void {
  clearHl();
  ...
  if (!inView) el.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
}

function clearHl(): void {
  if (_hlEl) { _hlEl.classList.remove('__amhl'); _hlEl = null; }
}

function escLoc(s: string): string {
  return s.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
}

function escLocDbl(s: string): string {
  return s.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
}
...
function getAccessibleName(el: Element): string {
  ...
  if (parentLbl) {
    const clone = parentLbl.cloneNode(true) as Element;
  ...
  const tag = el.tagName.toLowerCase();
  if (['button', 'a'].includes(tag)) return visibleText(el);
...
function getAssociatedLabel(el: Element): string {
  ...
  if (lbl) {
    const clone = lbl.cloneNode(true) as Element;
    clone.querySelectorAll('input, select, textarea, button').forEach(c => c.remove());
...
function buildCssSelector(el: Element): string {
  ...
  if (name) sel += `[name="${escLocDbl(name)}"]`;
  else if (ph) sel += `[placeholder="${escLocDbl(ph)}"]`;
  else if (al) sel += `[aria-label="${escLocDbl(al)}"]`;
  else {
...
function buildXPath(el: Element): string {
  ...
  while (cur && cur.tagName !== 'HTML') {
    ...
    parts.unshift(sibs.length > 1 ? `${t2}[${sibs.indexOf(cur as Element) + 1}]` : t2);
    cur = parent;
  }
  return '/' + parts.join('/');
}

function generatePlaywrightLocators(el: Element): string[] {
  const tag = el.tagName.toLowerCase();
  ...
  return out
    .sort((a, b) => a.p - b.p)
    .filter(l => { if (seen.has(l.c)) return false; seen.add(l.c); return true; })
    .map(l => l.c);
}

function generateSeleniumLocators(el: Element): string[] {
  const tag = el.tagName.toLowerCase();
  const out: Array<{ c: string; p: number }> = [];
  const id  = el.id;

  if (id) out.push({ c: `By.id("${escLocDbl(id)}")`, p: 1 });

  const name = el.getAttribute('name');
  if (name) out.push({ c: `By.name("${escLocDbl(name)}")`, p: 2 });
  ...
  if (tag === 'a') {
    ...
    if (lt && lt.length <= 60) out.push({ c: `By.linkText("${escLocDbl(lt)}")`, p: 3 });
  }
...
function scanPageLocators(): LocatorResult[] {
  clearHl();
  locatorMap.clear();
