...
window.fetch = async function (input, init) {
    ...
    const method = (init?.method
        ...
        ?? 'GET').toUpperCase();
    const requestHeaders = headersToObj(init?.headers
...
XMLHttpRequest.prototype.open = function (method, url, ...rest) {
    this[P + 'm'] = method.toUpperCase();
    this[P + 'u'] = typeof url === 'string' ? url : url.href;
    this[P + 'h'] = {};
    ...
    this.addEventListener('loadend', function () {
        ...
        try {
            for (const line of (this.getAllResponseHeaders() ?? '').trim().split('\r\n')) {
                ...
                if (sep > 0)
                    responseHeaders[line.slice(0, sep).toLowerCase()] = line.slice(sep + 2);
            }
        }
